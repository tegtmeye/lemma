...
BOOST_AUTO_TEST_CASE( all_key_value_test )
{
  ...
  BOOST_REQUIRE(detail::contents_equal<string_type>(vm,
    variable_map_type{
      {_LIT("key"),{string_type(_LIT("-f"))}},
      {_LIT("key"),{string_type(_LIT("-f42"))}},
      {_LIT("key"),{string_type(_LIT("--bar"))}},
      {_LIT("key"),{string_type(_LIT("--bar=43"))}},
    }));
}
...
BOOST_AUTO_TEST_CASE( key_value_with_embedded_operand_test )
{
  ...
  options = options_group_type{
    co::make_option(_LIT(""),
      co::value<string_type>().implicit(_LIT("floo")),_LIT("case 16")),
    co::make_operand(_LIT("key"),_LIT("case 16"),co::value<string_type>())
  };
...
//   std::cerr << detail::to_string<string_type>(vm);
  ...
  BOOST_REQUIRE(detail::contents_equal<string_type>(vm,
    variable_map_type{
      {_LIT("key"),{string_type(_LIT("operand1"))}},
      {_LIT("key"),{string_type(_LIT("operand2"))}},
